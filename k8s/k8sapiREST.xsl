...
<k8sapi version="{@version}" date="{@date}" type="rest">
    ...
    <!-- Read/Write Operations for xxx: request/path/query/response -->
...
<operations>
    ...
    <xsl:template match="operation">  <!--  Following: request; path/query/body parameters; response -->
       <xsl:variable name="node" select="preceding-sibling::operations[1]"/>
...
<operation>
     ...
     <xsl:attribute name="operations"><xsl:value-of select="$node/@name"/></xsl:attribute>
