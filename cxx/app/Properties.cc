...
bool PropertiesGroup::hasGroup(const String & name) const
{
	return groups_.find(name) != groups_.end();
}
const Properties * PropertiesGroup::getGroup(const String & name) const
{
	auto iter = groups_.find(name);
	return groups_.find(name) == groups_.end() ? nullptr : iter->second;
}
Properties * PropertiesGroup::getGroup(const String & name)
{
	auto iter = groups_.find(name);
...
void PropertiesGroup::removeGroup(const String & name)
{
	auto iter = groups_.find(name);
...
void PropertiesGroup::output(std::ostream & out) const
{
    for (auto & name : names_)
    {
        const Properties * props = getGroup(name);
        if (props != nullptr)
            props->output(out);
    }
}
